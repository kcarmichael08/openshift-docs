...
. To get the password for the `admin` user, you can use one of these methods:
...
----
. Find the link to the {product-title-short} portal by completing one of these actions:
* Use the {osp} CLI command:
+
[source,terminal]
----
$ oc -n stackrox get route central -o jsonpath="{.status.ingress[0].host}"
----
* Use the {product-title} web console to find the link to the {product-title-short} portal by performing the following commands:
.. Go to *Networking* -> *Routes*.
.. Find the *central* Route and click the {product-title-short} portal link under the *Location* column.
