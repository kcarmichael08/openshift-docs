...
. {product-title-short} installs a lightweight version of Scanner on every secured cluster when you install {product-title-short} on {ocp} by using the Operator. The lightweight Scanner enables the scanning of images in the integrated {product-registry}. When you install {product-title-short} by using the Helm install method with the default values, the lightweight version of Scanner is not installed. To install the lightweight Scanner on the secured cluster by using Helm, you must set the `scanner.disable=false` parameter.
