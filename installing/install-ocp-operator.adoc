...
[role="_abstract"]
{product-title} (RHACS) installs a set of services on your {ocp} or Kubernetes cluster. This section describes the installation procedure for installing {product-title} on your {ocp} or Kubernetes cluster by using an Operator.
...
. Generate and apply an init bundle. The init bundle contains the secrets that provide linking between Central and the secured clusters.
. Install the {product-title} Operator in all clusters that you want to monitor.
. Configure and deploy the `SecuredCluster` custom resource in each individual cluster that you want to monitor.
...
[id="generate-init-bundle-operator"]
