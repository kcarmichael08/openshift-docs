...
= Installing by using the `roxctl` CLI
...
[role="_abstract"]
{product-title} installs a set of services on your {ocp} cluster.
This topic describes the installation procedure for installing {product-title} on your {ocp} cluster by using the `roxctl` CLI.

[WARNING]
====
For production environments, Red Hat recommends xref:../installing/installing_helm/install-helm-customization.adoc#acs-install-helm-customization[Installing {product-title} by using Helm charts].
====

High-level installation flow:

. Install the `roxctl` CLI.
. Use the `roxctl` CLI interactive installer to install the xref:../architecture/acs-architecture.adoc#centralized-components_acs-architecture[centralized components] (Central and Scanner).
...
* xref:../installing/installing_helm/install-helm-customization.adoc#acs-install-helm-customization[Installing {product-title} with customizations using Helm charts]
